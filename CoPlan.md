...
|---|---|---|
| **A. Fixed now** | Landed in the working tree, with tests. Carries no work item. | Group 1, decode correctness, is complete: every field the three vendors state is decoded, the model name is decomposed into line, generation, version, and gradation, and coverage is reported from the store rather than from run-time counters. Two dead columns went with them -- `content_derivations.input_content_id` and `output_content_id`, one of them `NOT NULL`, requiring a content object per processed value that nothing read. `global_id` is `entity_id`, `sources.source_uri` is `source_path` (it held a bare path on all 407 rows, no scheme), `source_fingerprint` is `read_source_revision` after the value it produces, and `evidence_resolver` is `source_verification` after the question it answers. Hashing was evaluated across all 55 call sites and five were removed as unnecessary -- the two content digests recorded per processing action and never compared, which also freed a `NOT NULL` foreign key and the content objects it required -- two written and never read (`source_locator_sha256`, `stdout_sha256`) and one naming a staging directory, now an index -- leaving identity, integrity, content addressing, and change detection. `fileio.file_state` gives the five modules that each rolled their own `(size, mtime_ns)` check one function, and adds the inode, without which a file replaced by rename with a copied mtime reads as unchanged. Names are settled in [CoNames](CoNames.md), and `harness_name` no longer carries a surface suffix -- `claude-code-cli` contradicted the decoded `surface_kind` on 351 of 374 Claude Sessions. Two columns that no vendor filled now do: `events.parent_event_id` resolves Claude's `parent_uuid` against `record_uuid` over the whole Session, giving 30,197 links where the column was wholly null, `tool_namespace` reads the MCP server from the tool name across all three vendors, and `tool_results.output_json` retains Claude's structured result on 9,455 of 9,956 rows, making `stderr`, `interrupted`, and `structuredPatch` queryable where the text projection had flattened them into one blob. Cursor serializes structure into a JSON string and Codex states a header of facts before its body -- `Process exited with code`, `Wall time`, `Original token count`, `Chunk ID` -- so all three are decoded and `output_json` covers 9,455 Claude, 20,510 Codex, and 45,231 Cursor results. **The Codex header also closes a status gap**: the exit code was read only from an embedded JSON form present on 79 results, while the header states it on 14,795, so unknown outcomes fell from 26,917 to 5,499. What each change established is recorded where it is checkable -- `schema/field-coverage-baseline.json` for accepted gaps, `schema/model-aliases.json` for the model vocabulary -- rather than retold here. |
| **B. Fix as WIP** | Actively in progress; the remaining work is named. | W45, W46, W52 |
| **C. Fix later** | Accepted, unblocked, not started. Grouped and ordered below. | W04, W07, W08, W09, W13, W47, W48, W49, W50, W51, W14, W15, W18, W19, W21, W24, W28, W31, W32, W38, W41, W42 |
