...
| **C. Fix later** | Accepted, unblocked, not started. Grouped and ordered below. | W54.3, W04, W07, W08, W09, W13, W47, W48, W49, W50, W51, W14, W15, W18, W19, W21, W24, W28, W31, W32, W37, W38, W39, W41, W42 |
...
| W54.3 | `model_configurations` becomes `model_params`; every `model_config_id` follows |
...
|---|---|---|
| 0. Fix the model | W54 -- W54.1 (decode) and W54.2 (aliases and tokenizers) **complete**; **W54.3** (schema and names) partly done, renames pending | Harness, vendor, provider, and model are four facts, not one. Every name below refers to one of them, so naming them before the distinction is settled fixes the wrong words. |
...
| 4. Land the wire format | **W25**, **W33**, **W34**, **W36**, **W54.3** | One regeneration, after the names are settled and the SQL is guarded. W54.3 joins it: the `model_params` rename touches the DDL, the contract, and all three mapping profiles. |
...
| W54 | High | **Partly done** | Separate harness, vendor, provider, and model. `vendor` is the company a harness comes from; `provider` is the company whose model answered; Cursor runs three providers' models under one harness. W54.1 (decode) and W54.2 (aliases and tokenizers) are **complete**, and `provider` now names four companies across 60 of 66 model parameter rows. W54.3 is partly done: its three zero columns are populated, and its renames wait on the regeneration batch. |
...
`haiku`, `fable`, `mython`, with `mythos` reserved and unobserved.
