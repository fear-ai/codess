...
|---|---|---|
| **A. Fixed now** | Landed in the working tree, with tests. Carries no work item. | **Group 1 (decode correctness) is complete: W01, W02, W37, W39.1, W39, and W12 all closed.** **This pass:** **W54.1** -- Claude `entrypoint` into `surface_kind` and `effort` into `reasoning_effort`, Codex `model_provider` joined to the model it was stated apart from, Cursor `unifiedMode` and `maxMode`; the work-item references in code comments were also removed, since an identifier CoPlan deletes on completion resolves to nothing for a later reader. **Earlier this pass:** `codess query --coverage` and `codess/coverage_report.py` (W12); Cursor client version and tool file paths (W37, W39); `RunTotals` and `CursorSelection` collapsing phase parameters (17/15/10 to 10/13/7); `_ingest_project` extracted, completing W44; three stale closure calls and a wrong return shape fixed; `_cursor_preflight` gained a real return annotation, verified to catch the bug that shipped; `get_composer_data`/`read_composer_data` removed; two free-standing duplicate blocks merged. **Earlier:** W40 Codex harness/surface decode; Codex `apply_patch` file paths and first Codex Artifacts; the Codex envelope builder verified over 96,730 Events; slug decoding by filesystem walk; `field_state.coarse`/`is_vacant` removed; `helpers` security-boundary tests; `tools/field_coverage.py` and its evidence-backed baseline; the test-naming convention; `decode_audit.py` in the validation sequence; `_backup_store` write-gated; read-only connection setup consolidated. |
| **B. Fix as WIP** | Actively in progress; the remaining work is named. | W45, W46, W52. **W01, W02, and W39.1 closed** -- W01's condition (a decode change validated *through* 11.3 rather than alongside it) was met by three changes; W02's code content is done with its manual review tracked as task 28; W39.1 closed both remaining status gaps. |
| **C. Fix later** | Accepted, unblocked, not started. Grouped and ordered below. | W54, W54.2, W54.3, W04, W07, W08, W09, W13, W47, W48, W49, W50, W51, W53, W14, W15, W18, W19, W21, W24, W28, W31, W32, W37, W38, W39, W41, W42 |
...
**That cost is what makes F a category rather than a queue.** Each of the
six items changes what a store records:
...
| W31, W32 | Every `global_id` gains a format qualifier and a new derivation |
...
|---|---|---|
| **1. Decode correctness** -- what the stores say is true | **Complete** | W39.1 closes four gaps already validated against sources. W37 adds the Cursor version those gaps need for attribution. W39 works the rest of the coverage sweep. W12 reports coverage, so it is last: a report built earlier describes an incomplete decoder. |
...
|---|---|---|
| 0. Fix the model | **W54**, and within it W54.1 (decode what is stated -- **complete**), **W54.2** (aliases and tokenizers), **W54.3** (schema and names) | Harness, vendor, provider, and model are four facts, not one. Every name below refers to one of them, so naming them before the distinction is settled fixes the wrong words. |
| 1. Settle the names | **W53** | Names every designator once, across DB, module, file, structure, method, and field. Every step below checks against it. |
...
| Rank | Group | Why here |
|---|---|---|
| 1 | **1. Decode correctness** -- **complete** | Its failure mode is silently wrong data, which every other group builds on and none of them detects. A reporting contract over a wrong decode reports wrongly, on schedule. W39.1 closed this pass, so what remains is the coverage sweep's tail and the loss report. |
...
| 3 | **2. Contract enforcement** (W04 → W13 → W14) | Baseline 2's substance, and W12 in group 1 reports against the profiles W04 enforces. Ranked below group 1 only because a coverage gap you can see beats a conformance check you have not written. |
...
|---|---|---|---|
| W01 | Critical | **Complete** | Audit source-type and Actor classification across CC/Codex/Cursor. Validated over 17 real Projects (142,363 Events): every Event classified, no contradictory pairing (13.4.9). Remaining: run the audit as part of the validation sequence rather than by hand. |
| W02 | Critical | **Complete** | Strengthen tool, context, compaction, model-setting, and agent/subagent decode. Every real-source gap found so far is closed: `file-history-delta`, image-only prompts, `invocation_kind`, Cursor subagent lineage, and now Codex file operations. The Codex envelope duplication is resolved: 15 sites wrote the same 20-key dict inline across 405 lines, now one `_base_event` builder, verified byte-identical over 96,730 real Codex Events. **Closed** on its code content; the manual review of the image-only classification is postponed as task 28 and tracked there rather than holding this item open. Historical text follows (13.4.9). |
...
| W34 | Normal | **Accepted** | Confine the algorithm name to `codess/hashing.py` -- not general code, messages, or documentation. All 120 remaining occurrences are wire-tied (4 columns, ~20 JSON keys, the identity prefix); replacement term is `digest`. Regenerate with W25/W33/W36 and W31/W32. |
...
| W37 | Normal | **Complete** | Decode Cursor's client version into `sessions.harness_version`. Claude and Codex populate it on 425 of 426 Sessions; Cursor populates 0 of 81, though its own store records the version. |
...
| W39 | High | **WIP** | Close the per-vendor field gaps a cross-vendor null-rate sweep found. **Two more closed this pass.** Cursor tool calls name the file they operate on under four spellings -- `target_file`, `file_path`, `relative_workspace_path`, `path` -- so one field read found none of them and `events.file_path` was null for every Cursor Event; 2,873 of 4,530 real tool calls carried one. Re-ingest gives 5,834 Events with a path and **671 Cursor Artifacts where there were none**. The sweep now reports 9 single-vendor gaps, down from 12, with `harness_version`, `release`, `file_path`, and `artifact_path` all closed. `model_turns.source_turn_id` was checked and is **not** a defect: Cursor records `requestId`, but every value is unique to one bubble -- 881 identifiers across 881 groups in a 12,000-bubble sample -- so it names a request, not a Model Turn spanning several records. Using it would assert a grouping the vendor does not make; Cursor turns are derived and `model_turns.boundary_source` already records `inferred` for all of them. Recorded in the baseline with its evidence. **`tools/field_coverage.py --fail-on-gap` now exits 0**: single-vendor gaps fell from 12 to 6 and every one is closed or explained, so a newly introduced gap fails the check. Measured after rebuilding all four Cursor Projects: 125,323 Cursor Events, 31,259 carrying a file path, 3,020 Artifacts, and a client version on every Session -- all four figures were zero. Original text: ten columns are wholly null for one vendor and populated for another; four are decode gaps confirmed against real sources, including zero Codex Artifacts and 26,917 Codex tool results with no outcome. |
...
|---|---|---|
| Absent for every vendor | 18 | No adapter writes it (W39's third class) |
...
| W54 | High | Planned | Separate harness, vendor, provider, and model. `vendor` is the company a harness comes from; `provider` is the company whose model answered. Cursor differs in 5 of 10 configurations. Split into W54.1 (decode what vendors already state), W54.2 (alias table and per-vendor tokenizers), W54.3 (schema and naming). |
| W54.1 | High | **Complete** | Decode the model and harness evidence vendors already state and Codess discards. **All four decodes landed and are verified by re-ingest of real Projects.** (1) **Claude `entrypoint` now sets `surface_kind`.** Re-ingesting one Project moved 353 Sessions from the constant `cli` to `api` 327, `cli` 16, `desktop` 10. (2) **Claude `effort` now sets `reasoning_effort`**, which held zero Claude rows; the field is top-level on the record, not inside `message`, so it is read there and an absent `message` no longer drops it. (3) **Codex `model_provider` now reaches the model.** The cause was sharper than "not decoded": Codex states provider and model on *different record types* -- `model_provider` on `session_meta` (468 records, no model) and `model` on `turn_context` (8,013 records, no provider) -- so a decoder reading one record at a time could never see both, and the store held a provider-only configuration beside a model-only one. Seeding the accumulated configuration from `session_meta` joins them: `(None, 'gpt-5.5', 'high')` became `('openai', 'gpt-5.5', 'high')`. A provider stated on a turn still wins over the Session-level one. (4) **Cursor `unifiedMode` and `modelConfig.maxMode` are read** from `composerData`, which the ingest path did not open at all -- only the `composerHeaders` table was read. `unifiedMode` also appears on bubble records but as an *integer* where the composer states a word, and across 3,984 real bubbles the integer co-occurred with `agent` alone, so only the composer's spelling is decoded rather than storing a constant of unknown meaning. **Measured over all 22 Projects after re-ingesting every one:** `surface_kind` is Claude `api` 327 / `cli` 23 / `desktop` 30, Codex `cli` 15 / `ide` 7, Cursor `ide` 86; `provider` is on 29 of 64 configurations against 12 of 58 before; `reasoning_effort` on 29 (`high` 25, `medium` 4) against 14; Cursor `interaction_mode` splits `chat` 17 / `agent` 17. `tools/decode_audit.py` exits 0 over all 22 with no inconsistency, and `tools/field_coverage.py --fail-on-gap` exits 0 with no unreviewed gap. **One baseline entry was added rather than a gap left to fail**: `model_configurations.reasoning_effort` is now empty for Cursor alone, because Claude and Codex populate it and Cursor states no effort field -- 3,000 sampled bubbles carry no reasoning key but `allThinkingBlocks`, which is generated content. The strength does sit inside Cursor's model label, but recovering it is name parsing and belongs to W54.2, not to this column. **Two corrections to this item's own evidence.** The `entrypoint` counts above are *per record* and invert *per Session*: `sdk-cli` is 1,656 records but **328 of 370 Sessions**, while `claude-desktop` is 43,186 records but 24 Sessions -- SDK Sessions are many and short, Desktop Sessions few and long. So the prediction "about half are recorded as CLI Sessions that ran under Claude Desktop" is a record-count claim; by Session the dominant misrecording was SDK. `surface_kind` is a `sessions` column, so the per-Session denominator is the operative one. And `sdk-cli` maps to **`api`, not `cli`**: its records carry `promptSource: sdk`, so the Session was driven programmatically, and mapping on the `cli` token would have flattened the majority class into the wrong surface. Promotion of a per-record field to a Session column is licensed by a check: across 370 real Sessions **none mixed two `entrypoint` values**, and the latest first statement of any Session fact was line 7. The exact vendor token is retained in Session metadata beside the mapped value, since three entrypoints map onto a shared four-value vocabulary. 19 tests added; 1,270 pass; no new lint findings. | Each stated field is stored; `surface_kind` and `reasoning_effort` vary where the source varies; a re-ingest changes the counts. |
| W54.2 | Normal | Planned | Add per-vendor model tokenizers and a loadable alias table. Cursor states a label rather than an API name (`claude-4.6-opus-high-thinking`), and token order differs per vendor -- Claude puts family second, Cursor third -- so one parser would misread one vendor to satisfy another. The alias table is **data, not code**, so a new model name does not require a release; comparison is case-folded. `model` keeps the vendor string verbatim; `model_api` carries the derived identifier for Cursor. Family vocabulary: `opus`, `sonnet`, `haiku`, `fable`, `mython`, with `mythos` reserved and unobserved. | A Cursor label resolves to an API name and a strength; an unknown name is recorded unresolved rather than guessed; adding an alias is a data edit. |
| W54.3 | Normal | Planned | Correct the schema and names once W54.1 and W54.2 have shown what is real. `vendor_name` records the company (Anysphere for Cursor, not `cursor`); `provider` is required where a Model Turn is recorded; `model_family`, `model_revision`, and `speed_tier` are populated or removed -- all three are zero of 58 today. Two facts nothing represents: **model substitution** (Claude `originalModel` with `fallbackModel` -- one model asked for, another used) and **absence of an explicit choice** (Cursor `modelName: "default"`, which is not an unknown model). **A third name to settle: `sessions.default_model_config_id`.** Of its three qualifiers two are earned -- `_id` marks a foreign key to a surrogate rowid, and `_config` names the referent, since the target is a configuration tuple and no model table exists to point at -- but `default_` asserts a fallback role the evidence does not support. It is populated for **22 of 22 Codex Sessions and 0 of 484 Claude and Cursor Sessions**, because only Codex states a session-level model choice while the other two state one per turn. So "default" invites the reading that 484 Sessions defaulted to something unrecorded, when they simply record the model elsewhere. `session_model_config_id` states where the value applies rather than claiming a fallback. Wire-format; joins the regeneration batch and follows W53. | Vendor and provider are separately queryable; a substituted model is visible as one; no model column is constant across every store. |

**Both words are already in the schema, and one is used wrongly.**
`sessions.vendor_name` holds `anthropic`, `openai`, `cursor` -- so two are vendors under this definition and the third is a harness name, since Cursor's vendor is Anysphere. `model_configurations.provider` is the right column for the second sense and is populated on 12 of 58 rows. So the fix is not new columns: it is `vendor_name` recording the company rather than the product, and `provider` being decoded rather than left null. A model name carries a **family** (Opus, Sonnet, Haiku, and now Fable and Mython for Claude), a **generation** (the Claude family is at 5; OpenAI at 5, most recent iteration 5.6), and sometimes a **strength** (High, Med, Low, and others). **Measured divergence.** Of Cursor's 10 model configurations, **5 name a provider other than its vendor** -- `claude-*` (Anthropic) and `grok-*` (xAI) against `composer-*` (Anysphere). Under one term the two facts are indistinguishable in half the rows of the one harness where it matters. **What the data shows.** Across real stores the Claude Code harness ran only `claude-*` models, Codex only `gpt-*`, but **Cursor ran `claude-4.6-opus-high-thinking`, `grok-4.5`, `grok-4.6`, and its own `composer-*`** -- three providers under one harness. So harness does not imply provider, and the earlier reading that treated `sessions.vendor_name` as the vendor of the work is wrong: an Anthropic Opus 4.6 turn is stored as `vendor_name=cursor`, `product_name=cursor-composer`, `provider=NULL`. **The schema is closer to right than the decode is.** `model_configurations` already declares `provider`, `model_family`, `model_name_exact`, `model_revision`, `reasoning_effort`, `speed_tier` -- the decomposition this model needs. Of 58 rows: `model_name_exact` 50, `provider` 12, `reasoning_effort` 14, and **`model_family`, `model_revision`, `speed_tier` all zero**. Nothing in the codebase parses a model name, so `opus`, `4.6`, and `high-thinking` sit opaque inside one string. **This supersedes several earlier assumptions**, which were formed before the distinction was stated: W40's reading of the four identity terms, W51's treatment of `vendor_name`, and the W48 note that vendor tags are merely redundant -- `vendor_name` is not redundant, it is *wrong* for any harness running another provider's model. **Confirm before implementing**: the family and strength vocabularies, whether generation is parsed from the name or carried separately, and whether provider is decoded per Model Turn or per configuration.

**How each searchable field is obtained differs by vendor, which is why this
is a per-adapter decision rather than one parser.** Measured against real
Sources:

| Harness | Records | Exact API name | Provider | Strength | Family and generation |
|---|---|---|---|---|---|
| Claude Code | `message.model` | yes -- `claude-sonnet-5` | not stated | not stated | parse: `claude-<family>-<gen>[-<rev>]` |
| Codex | `payload.model`, `effort`, `model_provider` | yes -- `gpt-5.6-sol` | **stated** (`openai`) | **stated** (`high`, `medium`) | parse generation from `5.6`; no family |
| Cursor | `modelInfo.modelName` | **no** | not stated | embedded (`high-thinking`) | parse, and the token order differs |

**The exact name is kept verbatim in every case** -- `model_name_exact` is
what was supplied to the API and is never reconstructed from parsed parts.
The searchable fields are additive: `provider`, `model_family`,
`model_revision`, `reasoning_effort` already exist for them.

**Two findings the table makes concrete.** Codex *states* provider and
strength as fields, so decoding them is a read, not an inference -- and both
are currently discarded (`provider` populated on 12 of 58 rows,
`reasoning_effort` on 14, while `effort` appears on 7,870 real records).
Cursor's `modelName` is **not** an API name: `claude-4.6-opus-high-thinking`
puts the generation before the family and appends a strength, where Claude
Code's own `claude-opus-4-8` puts the family first and carries no strength.
So a single tokenizer would misread one vendor to satisfy the other, and
`model_name_exact` for Cursor is a harness label rather than an API
identifier -- a distinction worth recording rather than flattening.

Confirm the family vocabulary before parsing: Claude currently shows `opus`,
`sonnet`, `haiku`, and `fable`, with `mython` named but not yet observed.


**A second sweep of all three Sources found more stated evidence than the
first pass credited, and it changes the decode plan from "parse names" to
"read what is already there".**

| Evidence | Claude Code | Codex | Cursor | Stored today |
|---|---|---|---|---|
| Exact model name | `message.model` | `payload.model` | `modelInfo.modelName` (a label, not an API name) | yes, 50 of 58 |
| Reasoning strength | **`effort`, top-level, 14,050 records, all `high`** | `effort`, 7,870 records (`high`, `medium`) | `maxMode` in `modelConfig`; also embedded in the label | **14 of 58** |
| Provider | not stated | **`model_provider`** (`openai`) | not stated | 12 of 58 |
| Harness surface | **`entrypoint`: `cli` 24,089, `claude-desktop` 23,792, `sdk-cli` 187** | `originator` (`Codex Desktop`, `codex_cli_rs`, ...) | `agentBackend` (`cursor-agent`) | Codex only, since W40 |
| Interaction mode | -- | `collaboration_mode`, `approval_policy`, `sandbox_policy` | `unifiedMode` (`agent` 116, `chat` 41), `forceMode` | `mode` on 11 of 58 |
| Model substitution | **`originalModel` + `fallbackModel`** | -- | `modelConfig.modelName` = `default` | not decoded |

**Two findings are immediate defects rather than future work.** Claude Code
states `effort` on 14,050 records and Codess stores `reasoning_effort` for
**zero** Claude rows -- the field is read from Codex and ignored from Claude,
so the same fact is decoded for one vendor and discarded for another. And
`entrypoint` distinguishes three surfaces in near-equal proportion, while
`sessions.surface_kind` holds the constant `cli` for all 400 Claude
Sessions: roughly half are recorded as CLI Sessions that ran under Claude
Desktop. That is the same defect W40 found and fixed for Codex, present in
Claude and unnoticed because the constant looked plausible.

**Two more are evidence nothing currently represents.** `originalModel` with
`fallbackModel` records a *substitution* -- one model asked for, another
used -- which no column expresses, and Cursor's `modelName: "default"`
records the absence of an explicit choice, which is different from an
unknown model. Both are worth a decision rather than a silent flattening.

**On configuration and aliases.** A supplied mapping file is the right shape
for Cursor: `modelConfig` gives `{modelName, maxMode}` and `unifiedMode`
gives the interaction mode, but neither yields a provider, and
`claude-4.6-opus-high-thinking` needs an alias table to reach
`claude-opus-4-6` with strength `high`. Case folding before comparison, per-
vendor tokenizers, and a loadable alias table are accepted; the table should
be data rather than code so a vendor's new model name does not require a
release. **`model` records what the vendor gave, verbatim; `model_api` is
added for Cursor to carry the derived API name**, so the label and the
identifier are never confused. Mythos exists but is not generally available,
so the family vocabulary is `opus`, `sonnet`, `haiku`, `fable`, `mython`,
with `mythos` reserved and unobserved.
 | Harness, vendor, provider, and model are four separately recorded facts; a model run under any harness names its provider; family, generation, and strength are queryable rather than embedded in a string; `model` holds what the vendor gave, verbatim; `model_api` carries the derived API name where a vendor supplies a label instead; Claude `effort` and `entrypoint` are decoded rather than discarded; a model substitution is recorded as one; no item assumes harness implies provider. |
| W53 | High | Planned | Compile the settled-names list before any rename lands. Six items now propose renames -- W25, W33, W34, W36, W50, W51 -- and each was reasoned about in isolation, which is how the current inconsistencies arose. **The list is the prerequisite, not the outcome.** It states, once, for every designator: the DB table and column, the code module and file, the structure or class, the method, and the field or variable that carry the same concept, so that different things get different names and the same thing gets one name everywhere. **What the evidence already settles.** *Countable entities are plural; mass nouns are singular* -- accepted, and `*_content` stays singular as English usage, so no `event_contents`. *Rowid naming is fine* -- `sources.id` as an integer rowid is unremarkable. *`product` is dropped*: required by `mapping-contract.json`, defined nowhere, and the only values are three literals we wrote. *`source` keeps its settled meaning* -- the upstream evidence container -- so `sessions.source`, which holds the adapter key `Claude`/`Codex`/`Cursor`, is renamed rather than `source` being redefined. **What the evidence newly shows.** `vendor_name` mixes companies with a product: `anthropic` and `openai` are companies, `cursor` is a product whose company is Anysphere. The Claude designation has **five spellings** for three distinct things -- `cc` (CLI token, store filename, adapter module), `"Claude"` (profile dict key), `claude-code` (product), `anthropic.claude-code` (composite), `anthropic` (company) -- and "claude" *also* appears inside model names from another vendor's store (`claude-4.6-opus-high-thinking` in Cursor), so a token match cannot distinguish product from model. `source_system_id` reads as a tool or product designator and is used as one: it is `vendor + "." + product` composed in the profile, not an identifier Codess derives. **The list must therefore separate**, per W54's corrected model: **vendor** (the company -- Anthropic, OpenAI, Anysphere), **harness** (the program -- Claude Code, Codex, Cursor), **provider** (whose model was engaged, which need not be the harness vendor), **model** with its family, generation, and strength, plus the local designators -- adapter module, store filename, CLI token. Currently seven concepts share three words, and "claude" alone denotes a harness, a company's product line, and a model family that appears in another harness's store. **Ordering**: this precedes the wire-format batch. Renaming twelve columns against six separately-reasoned proposals would repeat the failure. | One document names every designator once; a reviewer can check any proposed rename against it; the six rename items reference it rather than each other. |
| W52 | Normal | WIP | Unify SQL construction and narrow where it lives. **Measured**: 186 statements across **24 modules**; **37 are built with f-strings in 11 modules**, and only 8 modules validate an interpolated name against the live schema. Four distinct interpolation patterns are in use and only the first is reviewed: (a) a predicate composed internally with bound parameters (`query_api`, the S608-exempt pattern); (b) a table or column name from the schema quoted by hand (`store.table_counts`, `schema_contract.column_names`, `value_survey`); (c) a fragment constant such as `cursor_source._BUBBLE_ROWS`; (d) an aggregate or projection list assembled per call (`query_reports`, 11 statements). **The risk is not injection** -- no site interpolates external input -- **it is drift**: a column renamed in the DDL is caught by (b) where `column_names` is consulted and silently produces an empty or failing query everywhere else, which is exactly what W25, W33, W34, W36, W50, and W51 are about to do to twelve column names at once. **Steps.** (1) One helper for quoting an identifier, replacing the four hand-rolled `replace('"', '""')` sites. (2) Every interpolated table or column name resolved through `table_names`/`column_names` so a rename fails loudly at the call site rather than returning nothing. (3) The three read-only audit modules that SELECT core tables from outside the query layer (7.3: 21 statements in 9 modules) either move behind `query_reports` or state at the call site why they read directly. (4) A test that parses every `execute` call and asserts each interpolated identifier appears in the released DDL, so the next rename cannot pass silently. Sequence before the regeneration batch, since its value is protecting exactly that change. | One identifier-quoting helper; every interpolated name checked against the schema; a renamed column fails a test rather than a query; SQL-executing modules reduced from 24 toward the store and query layers. |
| W51 | Normal | Planned | Resolve the source-identity naming, measured against peer projects rather than against our own v1. **v1 is not evidence**: `schema/legacy/coschema-v1.sql` has two tables, was generated, and was never reviewed or approved, so it cannot settle a convention. Four other local SQLite schemas can: `Misses` (48 tables), `OSINT` (4), `spank-py` (3), `spank-rs` (1). Of 56 tables across them, 11 are singular and **every one is a mass noun** -- `metadata`, `evidence`, `coverage`, `usage`, `provenance`, `audit`, `store_meta`. Countable-entity tables are plural without exception. **That settles CoSchema's four `*_content` tables**: `event_content` averages 1.27 rows per Event, so it is a countable set and should be `event_contents` -- or, better, renamed to what it holds. `store_meta` is correctly singular by the same rule. **`_id` carries four incompatible formats**, which is the sharper defect: `sessions.id` is a vendor UUID, `sessions.global_id` a `codess:session:sha256:` derivation, `sources.id` a bare SQLite rowid, `sessions.source_system_id` a dotted literal `anthropic.claude-code`. A reader cannot tell from the suffix whether a value is derived, assigned, or borrowed. **`source_system_id` is not an identifier under that reading** -- it is `vendor + "." + product` composed in the mapping profile, so `source_system` or `source_system_key` says what it is. **Superseded in part by W54**: `vendor_name` is not merely a redundant tag but the wrong fact for a harness running another provider's model, so its disposition is W54's rather than this item's. **`product` is required by `mapping-contract.json` and defined nowhere**: no vocabulary, no examples, no CoSchema entry, and `sessions.product_name` holds only the three literals the profiles supply. Either define it or drop it with W40. **`sessions.source` is the `SOURCE_PROFILES` dict key** -- `Claude`, `Codex`, `Cursor`, confirmed across 601 real Sessions -- so `adapter_key` names it honestly and frees `source` for the Source entity the glossary defines. Wire-format; batch with the regeneration. | One suffix rule states whether a value is derived, assigned, or borrowed; `source` names the Source entity only; every contract-required field has a definition and examples; plurality follows the mass-noun rule the peer schemas already use. |
...
| W44 | Normal | **Complete** | Extracted `ingest_cmd.run`'s two large blocks, completing W06's remaining step. **`run` fell from 692 lines and F (114) to 169 lines and D (23)**, reading as its four phases; `_cursor_preflight` is D (24) and `_ingest_project` F (62). **The extraction exposed four defects the inline blocks had hidden**, all of the same kind -- code that was correct inside `run` and wrong once it was not: `_cursor_preflight`'s failure path returned a bare `1` where the caller unpacks `(code, temporary)`; three sites called `cleanup_cursor_cohort`, `run`'s closure, from functions where it no longer resolves. All fixed. Two further gaps found by review rather than by failure: the return annotation was a string literal so mypy never checked it (now a real annotation, verified to catch the bare-`1` bug), and `_ingest_project` fell off its end implicitly (now an explicit `return None` distinguishing "continue" from "stop"). Coverage: the CLI suite runs ingest through subprocesses, so in-process measurement reported 41% where the true figure is 83%; the one real gap was the whole preflight, skipped because every Cursor CLI test passes `--force`. Two tests now cover the marker-cache reuse path and the unstable-container path that the before/after bracket exists for. |
...
| W12 | Normal | **Complete** | Report source-to-common coverage, loss, and unknown shapes. `codess query --coverage` derives them from the store rather than from run-time counters that printed to stderr and were discarded. |
...
| **W44** | Extraction found three calls to `run`'s `cleanup_cursor_cohort` closure from functions where it no longer resolved, plus a bare `1` where a tuple was required -- all correct while the state was implicit in one scope | Complete |
...
10-parameter one. W44 is done and is the evidence: making the state explicit
...
| **W12** | Coverage, loss, and unknown-shape reporting. A result a reader can challenge has to state what it did *not* map. |
...
**Order within the baseline.** W04 precedes W12: a coverage report derives
...
**Relation to W12.** W12 reports coverage, loss, and unknown shapes. Two of
the five steps feed it directly and are the ordering constraint:

- **W04.4 supplies W12's input.** A loss report states what was *not* mapped.
  Diagnostic rows are that record; without them W12 has to re-derive
  non-conformance by re-decoding, which is a second decode path.
- **W04.3 makes W12's counts comparable across vendors.** If Claude raises
  where Cursor tolerates, a coverage figure means something different per
  vendor and the cross-vendor comparison W12 exists to support is unsound.
- **W04.1 and W04.2 do not feed W12** and could land independently. W04.5
  overlaps W12 in presentation and should be folded into it if W12 lands
  first.

This is what "W04 precedes W12" means concretely: W12 built first would report
...
W12's coverage reporting exists to surface.
...
(W12) rather than repairable as decode.
...
| Query and contract surface | W04, W05, W12, W13, W17 | Results are unclear or unverifiable | Low to medium |
...
**W04, W05, W12, W13, W17.** Mixed maturity: W04 and W05 are Planned
with clear scope, W12 and W13 are TODO, and W17 is Under review awaiting
a consumer.
...
|---|---|---|---|---|
| W01 | Critical | **Complete** | Audit source-type and Actor classification across representative Claude Code, Codex, and Cursor Sessions. Validated over every real Project on the development machine -- 17 published store sets, 142,363 Events -- with no null Actor kind, content role, or origin kind, and none of the four contradictory pairings the audit checks (13.4.9). `tools/decode_audit.py` makes the check repeatable and content-free, is step 9 of 11.3's validation sequence, and covers Session relations -- a relation without a parent, a parent without a relation, and a self-parenting Session are three of its nine invariants. **Closed.** The condition it held open for is met: this pass changed decode in three places -- Codex `apply_patch` file paths, Codex `harness_name`/`surface_kind`, and the Codex envelope builder -- and each was validated through 11.3 step 9 rather than alongside it, with `tools/decode_audit.py` exiting zero over Claude, Codex, and Cursor stores (49,467 Events, no inconsistency). | Fixtures and real-source checks agree on Actors, roles, origins, relations, and source-accounting totals. |
| W02 | Critical | **Complete** | Strengthen tool, context, compaction, model-setting, and agent/subagent decode. Delivered against real Sessions: `file-history-delta` decodes as harness product state; image-only user messages decode as bounded attachment prompts; `invocation_kind` is derived from whether a request record exists (41,712 model-requested against 800 harness-observed where the column held one constant); and Cursor subagent Sessions now name the parent their header records, which closed a relation asserted without its evidence. Compaction, rollback, injection, and Session relations are reported by `tools/decode_audit.py` with four invariants that hold across 142,470 Events. Remaining: a manual review of whether the image-only classification reads correctly in context, and the repeated per-shape envelope in `adapters/codex` that duplicate detection surfaced (13.4.9, experiments/structural-analysis-tools.md). | Each supported family has exact source evidence, mapping, partial/malformed coverage, diagnostics, and an explicit validation basis. |
...
| W25 | Normal | Planned | Resolve CoSchema time-column naming. The defect is real and narrow: `started_at` is `REAL` in `sessions` and `tool_invocations` but `TEXT` in `processing_runs` -- one name, two representations, in one schema. **The earlier remedy was wrong, and is withdrawn.** `_atms` names the *storage encoding*, which is exactly what a column name should not do: it makes a representation change a rename across the DDL, contract, query paths, and every consumer, and it says nothing a reader wants to know. Reading the columns by *provenance* instead shows the split is already there and is not about types at all. Enumerating all nineteen time columns from the DDL gives three groups: Codess-recorded TEXT (`observed_at` ×3, `ingested_at` ×2, `created_at`, `asserted_at`, `processing_runs.started_at`, `completed_at`); source-reported REAL (`sessions.started_at`/`ended_at`, `tool_invocations.started_at`/`ended_at`, `event_at`, `record_at`, `timestamp`); and filesystem-observed REAL (`source_mtime` ×2). **`started_at` is the only name spanning two groups** -- every other name is unambiguous, which bounds the *rename* to three columns. **The larger finding is that nineteen time columns is far more than the sources justify.** Vendors supply approximately one time each: `timestamp` on every Claude and Codex record, plus Codex `started_at`/`completed_at` on a minority. Measured over 21 store sets, `events.event_at` and `events.timestamp` are **byte-identical across all 250,427 rows**, and `sources.observed_at` and `ingested_at` across all 422 -- two exact duplicate pairs. `tool_invocations.ended_at` is null in all 85,840 rows and no vendor reports an invocation end. `processing_runs.started_at`/`completed_at` are empty but **not** dead: `record_processing_run` is called from `ingest_publication` whenever a content policy is supplied, and both are test-covered, so the table is empty because the policy is opt-in. Four columns are ordinary row bookkeeping on unrelated tables and two record filesystem mtime, taken when the read starts so it pairs with the bytes actually read. That leaves about six genuinely distinct time facts, close to what the sources support. **CoSchema 5.1.1 states the resolution: nineteen columns become seven.** Six removals -- three duplicate pairs (`events.timestamp`, `sources.ingested_at`, `sessions.ingested_at`, each byte-identical to its partner in every row), two derivations (`sessions.started_at`/`ended_at` are exactly `MIN`/`MAX(events.event_at)` in all 497 Sessions), and one unwritten column (`tool_invocations.ended_at`) -- plus three unread row-creation stamps on derived tables. One rename survives (`tool_invocations.started_at` → `source_started_at`), because the other half of the collision is removed rather than renamed. Every survivor answers a question no other column answers, which is the standard the rest failed. Land the removals with the rename so no column is renamed on its way out. The representation follows the provenance, so naming the provenance fixes the collision and states something durable. Candidate: keep `_at` for times Codess recorded and adopt a `source_` prefix for times a vendor supplied, which follows the precedent `source_mtime` already sets in this same schema; `sessions.started_at` becomes `source_started_at` and the `processing_runs` column is unchanged. Decide the exact spelling against CoSchema 5.1 before landing. Breaking schema change; regenerate stores rather than migrate. | No column name denotes two representations; a name states who reported the time, not how it is stored; changing an encoding does not force a rename; the DDL, contract, and query paths agree. |
...
| W34 | Normal | **Accepted** | **Decision: the algorithm name appears only inside `codess/hashing.py`. Not in general code, not in messages, not in documentation.** This is the stronger of the two options 13.4.8 weighed, and it is chosen because the weaker one -- keeping the name on integrity fields a reader recomputes -- justifies itself by a habit rather than a need: a reader recomputes with `codess_hash`, which already knows the algorithm, so naming it in the field taught them nothing they could act on. **What this costs is now measured: every one of the 120 remaining occurrences is wire-tied.** There is no residue of code-only naming left to clean -- the two pure aliases were the whole of it and are already removed -- so the rename is 4 schema columns and 2 indexes, roughly 20 JSON keys in receipts, manifests, and catalogs, and the `codess:...:sha256:` prefix on every stored identity. The replacement is the neutral `digest`: `content_sha256` becomes `content_digest`, `selection_sha256` becomes `selection_digest`, `codess:content:sha256:` becomes `codess:content:digest:`. Width and algorithm remain recoverable from `hashing`, which is the module that would change if either did. **Sequencing: this is a wire-format change and must join the W25/W33/W36 regeneration, and it overlaps W31/W32, which rewrite the same identity prefix.** Landing W34's prefix change separately from W31's would rewrite every identity twice. Reviewed classification of the 120 follows. (1) **Wire format, 4 schema columns plus 2 UNIQUE constraints and 2 indexes** -- `content_sha256` on `sources`, `content_objects`, and `artifacts`, plus `policy_sha256` on `processing_runs`. Renaming these is a regeneration, so it belongs with W25/W33/W36 and not with the rest. (2) **Stored value prefixes** -- every `global_id` reads `codess:session:sha256:...`, and W31/W32 already change that prefix, so this class is decided *there*, not here; W34 should not re-decide it. (3) **Recomputable integrity fields** -- `stored_sha256`, `manifest_sha256`, `file_sha256`: a reader verifies these by recomputing, so the algorithm name tells them which function to run, and 13.4.8's proposal to keep it here is well founded. (4) **Code-internal names with no reader** -- local variables and private aliases. Two were pure indirection, `_sha256 = hash_file` in `schema_contract` and an identical unused alias in `baseline_validation`; both are **removed** (123 → 120), which is the entire safely-reducible content of this item. What remains blocked is only class 4's tail -- locals like `plan_sha256`, `resolved_sha256`, `selection_sha256` -- and the question is narrower than first stated: not "where may an algorithm name appear", but "should a local variable holding a digest name the algorithm when nothing recomputes it from that name". Classes 1 and 2 are owned by other items; class 3 is settled. | A reviewed rule states where an algorithm name may appear; code outside `hashing` follows it; changing the algorithm touches one module plus the wire-format change W25/W33/W36 already schedules. |
| W36 | Normal | Accepted | Split the `product_state` family, and rename what survives. **`product` is already a defined term, and this is not it.** The mapping contract requires a `product` field on every released profile, where it names the vendor application: `claude-code`, `codex`, `cursor-composer`. `product_state` therefore reads as "state of Claude Code", which is a claim the term is formally assigned to make elsewhere in the same schema. **Claude has no name for this family.** Its records are distinct top-level `type` values -- `attachment`, `mode`, `ai-title`, `custom-title`, `permission-mode`, `file-history-delta`, `file-history-snapshot`, `last-prompt` -- and the string `product` appears in no key of any Claude record. The grouping is entirely ours, which means no vendor term is being preserved and nothing constrains the choice but accuracy. **The naming question is therefore a decomposition question.** `state.product` is the largest single kind in Claude -- 19,528 Events, more than `tool.call` -- and it is Claude-only: Codex and Cursor emit nothing in the family, so it is not a common category that two vendors happen to fill differently. Its nine subtypes are not one thing. They are at least three: **Session labels** (`ai_title` 3,772, `custom_title` 2,247, `agent_name` 1) that name the Session and change over its life; **harness settings** (`mode` 2,690, `permission_mode` 1,264) that record how the harness was configured to act; and **content and position markers** (`context_attachment` 4,683, `last_prompt_marker` 3,748, `file_history_snapshot` 904, `file_history_delta` 219) that point at material or a place in the transcript. Renaming the union to `event_state` would put a better label on a category that should not exist -- a reader filtering `state.product` gets titles, permission settings, and file diffs in one result set, and no query can separate them without knowing the subtype vocabulary, which nothing declares. `event_kind` is a declared **open vocabulary** (`contract.json`), so adding kinds is the expected move rather than a schema break. **Accepted: four kinds, not three.** The three-way split was proposed from what the subtypes *are called*; checking what they *carry* refutes one of the three and confirms the other two. Across 19,479 real Events: `session.label` rows carry `content` and no metadata (6,012 of 6,012); `harness.setting` and the marker group carry metadata and no content. So content-versus-metadata cleanly separates labels from the rest, and the metadata *shape* separates the rest further. `mode` carries `{mode}` and `permission_mode` carries `{permission_mode}` -- one setting each. `context_attachment` carries `{attachment_type, item_count, has_content, is_initial, parent_uuid}` and `file_history_snapshot`/`file_history_delta` carry backup and tracking fields: all three describe *material the harness attached or tracked*. But `last_prompt_marker` carries only `{content_len}`, and its source record (`last-prompt`) holds `lastPrompt`, `sessionId`, and `leafUuid` -- a pointer to a position in the transcript, not a description of material. Grouping a position pointer with file diffs would repeat the error being fixed, one level down. The accepted partition is: **`session.label`** (`ai_title`, `custom_title`, `agent_name` -- 6,012), **`harness.setting`** (`mode`, `permission_mode` -- 3,938), **`content.attachment`** (`context_attachment`, `file_history_snapshot`, `file_history_delta` -- 5,789), and **`session.marker`** (`last_prompt_marker` -- 3,740). Four kinds each answer one question and each is separable by a field a reader can see, which is the test the current single kind fails. The mapping rule `claude.product-state` and its `events.state.product` target split the same way. Wire-format change; batch with W25 and W33. | Three kinds, each answering one question, replace one that answers none; a reader can select titles without receiving file diffs; the released Claude profile names each; the subtype vocabulary under each kind is declared rather than implied. |
| W37 | Normal | Planned | Decode the Cursor client version into `sessions.harness_version`. The field is populated for 403 of 404 Claude Sessions and 22 of 22 Codex Sessions, carrying values like `2.1.156` and `0.120.0`; it is null for all 81 Cursor Sessions. This is not a source limitation: Cursor's `globalStorage/state.vscdb` records `cursor.startupMetrics.lastVersion`, `cursor/localLastActivityClientVersion`, and `releaseNotes/lastVersion`, all reading `3.15.6` on the development machine, and `composer.composerHeaders.version` carries a separate header-schema stamp. Without it, a Cursor decode gap cannot be attributed to a release, which is the evidence every "vendor formats evolve independently" claim in the documentation depends on. Note the values are *current* client state rather than per-Session evidence, so decoding must record what the marker actually attests -- the version observed when the Source was read -- rather than asserting the Session ran under it. **Landed.** `cursor_source.get_client_version` reads the global store, trying `cursor.startupMetrics.lastVersion` first and falling back to the update and release-notes keys; `ingest_sources` reads it once per Source rather than per Session and passes it as `release`, which `store` maps to `harness_version`. Re-ingesting the Cursor Projects fills every Session with `3.15.6` where all were null. **The caveat is recorded in the function, because it changes what a reader may conclude:** Cursor writes its *current* version, not a per-Session one, so this attests the version observed when the Source was read. A Session decoded today from a Composer written months ago carries today's client version, and every Session in one store carries the same value by construction. Claude and Codex differ -- both state a version per Session -- so `harness_version` now means per-Session evidence for two vendors and per-observation evidence for the third. That asymmetry is worth a CoSchema note when W12 reports coverage. | Cursor Sessions carry a version whose meaning is stated; a decode gap can be attributed to an observed client release; the marker's limits as evidence are recorded rather than implied. |
...
| W39 | High | Planned | Close the field-coverage gaps `tools/field_coverage.py` reports. The sweep classifies every column by *which* vendors hold no value, because the scenarios are different findings, and over 21 real Project store sets it reports **38 columns in three classes**. **Twelve are empty for exactly one vendor and populated for others** -- the strongest signal, since the column is demonstrably decodable: `source_status` across `events`, `tool_invocations`, and `tool_results` is null for Claude while Cursor fills all 54,752 and Codex 2,352; `harness_version` and `release` are null for Cursor (W37); `model_turns.source_turn_id` is null for Cursor while Claude and Codex fill 100%; `service_tier`, `archive_source`, `parent_session_id`, `session_relation_kind`, `sources.content_sha256`, and `mapping_diagnostics.source_value` complete the class. **Eight are populated for exactly one vendor** -- `events.file_path` and `artifact_path` for Claude only, and `model_configurations.provider`, `mode`, `reasoning_effort` plus `sessions.default_model_config_id` for Codex only, which is where the Codex Artifact gap sits. **Eighteen are populated by no vendor at all**, including `events.parent_event_id`, `event_content.start_offset`/`end_offset`, `tool_invocations.ended_at`, `tool_results.output_json`, `content_objects.privacy_class`, and `model_configurations.model_family`/`model_revision`/`speed_tier`. That last group is not a decode gap but is the more awkward finding: `--model-family`, `--model-revision`, and `--speed-tier` are CLI filters over columns no store has ever populated, so each can only return nothing, and README's model-configuration bullet advertises all three. Each of the eighteen needs a decision -- decode it, or state in CoSchema that it is reserved and remove the filter that implies otherwise. **The earlier manual pass found ten of the thirty-eight**, because it looked only at `sessions` and `events`; the tool covers every table, which is the point of having one. | Every column is decoded, or recorded as reserved with its reason; no CLI filter selects on a column no adapter writes; `--fail-on-gap` keeps the single-vendor class from growing silently. |
| W39.1 | High | **Complete** | Validate and close the four confirmed decode gaps, the subset of W39 checked against real vendor records rather than inferred from null rates. **Gaps 1 and 2 are closed.** Codex `apply_patch` calls now decode their file path: the earlier claim that "143 Codex tool calls carry a `file_path`" was a substring match against patch *content* and was wrong -- Codex passes no path in any tool argument, `exec_command` carrying a shell string and `apply_patch` an envelope. The path is in the envelope's `*** Add|Update|Delete File:` headers, and **all 4,639 real `apply_patch` calls carry one, naming 5,722 file operations**. `_patched_file` extracts the first (the column holds one; `tool_input` retains the rest). Re-ingesting a real Codex Project moved `events.file_path` from 0 to 8 and **`artifacts` from 0 to 1 -- the first Codex Artifact the system has ever recorded** -- and the coverage sweep confirms `file_path`/`artifact_path` left the Claude-only class, dropping that group from 8 columns to 6. **Gaps 3 and 4 are now closed too.** (3) Claude stated the outcome directly via `is_error` on the result block, but `source_status` recorded only `application_error` -- a text-pattern inference used for MCP results alone -- so it was null on all 470 failure and denial Events. It now records `is_error`; re-ingest gives 95 failed and 12 denied with the source value, `normalized_status` unchanged. (4) Codex embeds `exit_code` as JSON inside the output body, which no field read reaches; `_exit_code_status` extracts it and `store._normalized_status` maps `exit_code:0` to succeeded and any other code to failed, leaving results with no code explicitly unknown rather than assumed successful. Re-ingesting every Codex Project gives 234 results carrying `exit_code:0`. The coverage sweep confirms `source_status` has left the single-vendor gap class entirely. Superseded detail follows. (3) `events.file_path` and `artifact_path` are null for all Codex and Cursor Events, but 143 Codex and 27 Cursor `tool.call` records carry a `file_path` in their tool input. (2) The `artifacts` table holds 1,001 rows for Claude and 2,937 for Cursor and **zero for Codex**, so no Codex Artifact is queryable though the paths are present. (3) `tool_results.source_status` is null for all 16,928 Claude results while the source records `is_error` on 2,292 of them, 98 being `true` -- `normalized_status` is correctly derived, so this loses the exact source value the schema exists to retain, not the outcome. (4) `tool_results.normalized_status` is null for **26,917 of 30,415 Codex results (88%)**; Codex reports the outcome as an `exit_code` embedded in output text rather than as a field, recoverable in about 11% of sampled outputs, so the honest fix records it where present and leaves the rest explicitly unknown. **Six are correct as-is and should be recorded so the sweep is re-runnable without re-litigating them**: `default_model_config_id` (only Codex names a session-level default), `archive_source` (Claude has no archive concept), `caused_by_event_id` (Claude alone records the causal link), `parent_session_id` and `session_relation_kind` (Codex records no relations), and `release` (tracked by W37 alongside `harness_version`). | Each of the four gaps is closed or explicitly recorded as unavailable with its reason; the sweep runs from `tools/decode_audit.py` so a new gap surfaces as a finding rather than as an absence nobody queries; the six explained nulls are declared so the report distinguishes "not applicable" from "not decoded". |
| W40 | High | Answered; now a decode gap | **The question is settled by looking at the sources, and the answer changes the item from a naming decision into a defect.** Of the four terms, `vendor_name` is real but never observed in a record -- it is correctly inferred from which vendor store a Session came from, and no vendor names its own company in a Session. `product_name` is the one genuinely derived term, computable from the store. But **`harness_name` and `surface_kind` are observable and currently misrecorded.** Codex `session_meta` carries `originator` with four distinct values across observed Sessions (`codex_cli_rs`, `Codex Desktop`, `codex-tui`, `codex_exec`) and `source` with two (`cli`, `vscode`); `get_session_metadata` already reads both into the Session metadata, and `store.SOURCE_PROFILES` then overwrites the columns with the constants `codex-cli` and `cli`. So a Codex Desktop or VS Code Session is stored as a CLI Session -- not a degenerate field but a wrong one, and precisely the cross-harness comparison 4.3 claims to support. Claude records `version` (already decoded as `harness_version`) and nothing naming its harness or surface, so its constants are correct. Cursor records a client version (W37) and no surface, so `cursor-ide`/`ide` is correct. **Resolution: three terms, not four. Decode `harness_name` and `surface_kind` from `originator`/`source` where a vendor supplies them, falling back to the profile constant where none does; keep `vendor_name`; and strike `product_name`.** `product_name` is a pure function of `source_system_id` -- `anthropic.claude-code` yields `claude-code` by taking the part after the dot -- so the column stored a value that could be computed from a column beside it. No record names it, no CLI flag filters on it, and only nine source references read it. A term that is neither observed nor queried nor independent is not a fourth axis; it is `source_system_id` spelled twice. Striking it also settles W36's premise cleanly: `product` then names one thing, the vendor application, and only in the mapping profile where the contract requires it. Dropping a column is a wire-format change, so it joins the regeneration batch. The degeneracy observed across 507 Sessions was an artifact of overwriting observed evidence with a constant, which is the opposite of the conclusion the counts first suggested. Superseded analysis follows. `sessions` carries `vendor_name`, `product_name`, `harness_name`, and `surface_kind`, plus `storage_format` and `source_system_id` on the same profile. Across 507 real Sessions they yield exactly **three distinct tuples** -- `anthropic`/`claude-code`/`claude-code-cli`/`cli`, `cursor`/`cursor-composer`/`cursor-ide`/`ide`, `openai`/`codex`/`codex-cli`/`cli` -- and every one is a constant in `store.SOURCE_PROFILES` keyed by the same three vendors. `harness_name` is `product_name` with a fixed suffix; `surface_kind` is that suffix; `source_system_id` is `vendor_name` plus `product_name`. None is a query filter: no CLI flag selects on them, and they are read back only by `query_api`'s session projection and `baseline_validation`. So six columns are stored per Session to express a three-valued fact already given by `source_system_id`. **Decide which of the terms are independent before any of them is reused.** The distinction the vocabulary is reaching for is real -- a vendor may ship several products, a product may run under several harnesses, and a harness may present several surfaces -- but nothing in the current data or query surface exercises it, and W36 needs `product` to mean one thing before it can rule the term out for the Event family. Options: keep all four and record that they are currently degenerate but are the axes a fourth source system would separate; or reduce to `source_system_id` plus `surface_kind` and derive the rest from the profile at read time. | The identity terms are defined against each other; each stored column is either independently observed or derived at read time rather than duplicated per Session; `product` has one meaning the schema can point at. |
...
|---|---|---|---|---|
| W12 | Normal | **Complete** | Report source-to-common coverage, loss, and unknown shapes. **`codess.coverage_report` derives all three from a published store**, exposed as `codess query --coverage`: admitted against classified Events with a recomputable ratio, the vendor record shapes seen and their counts, and diagnostic reasons split by level. **Deriving from the store rather than from run-time counters was the design decision.** The ingest command printed eleven diagnostic counts to stderr and discarded them -- they could not be queried later, compared between runs, or attached to a result, and they answered questions about one process rather than about the Project. They had also drifted: the printed list named eleven counters while adapters produce twenty-one, so ten were counted and never reported, and one printed name no longer existed. A report derived from stored evidence removes the second list that has to be kept in step. **The level split is the substantive reporting decision.** A `source` or `record` diagnostic means something did not become an Event; a `field` diagnostic means an Event exists with a value missing. Summing them reports a Project as lossier than it is -- real stores show thousands of field diagnostics against zero record loss -- so the report separates them and the tests pin that they are never added. Measured across vendors: Claude 28,816 Events fully classified with 341 field diagnostics, Codex 9,410 with 4,024, Cursor 19,309 with 1,423, and no record-level loss in any. | The report derives from profiles, diagnostics, and selected source observations. |
