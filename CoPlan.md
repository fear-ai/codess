...
| **C. Fix later** | Accepted, unblocked, not started. Grouped and ordered below. | W04, W07, W08, W09, W13, W47, W48, W49, W50, W51, W14, W15, W18, W19, W21, W24, W28, W31, W32, W37, W38, W39, W41, W42 |
...
changes what a store records. W54.3 is not yet an F item -- its other work is
open -- but its rename is settled and rides the same rebuild:
...
| W54.3 | **Landed.** `model_configurations` became `model_params`, `model_family` became `model_gradation`, and four axis columns were added |
...
|---|---|---|
| 0. Fix the model | W54 -- **complete**: W54.1 (decode), W54.2 (aliases and tokenizers), W54.3 (schema and names) | Harness, vendor, provider, and model are four facts, not one. Every name below refers to one of them, so naming them before the distinction is settled fixes the wrong words. |
| 1. Settle the names | W53 -- **complete**, delivered as [CoNames](CoNames.md) | Names every designator once, across DB, module, file, structure, method, and field. Every step below checks against it. |
...
| 4. Land the wire format | **W25**, **W33**, **W34**, **W36** | One regeneration, after the names are settled and the SQL is guarded. W54.3's `model_params` rename already landed with its own regeneration. |
...
| W54 | High | **Complete** | Separate harness, vendor, provider, and model. `vendor` is the company a harness comes from; `provider` is the company whose model answered; Cursor runs three providers' models under one harness. All three parts landed: W54.1 decoded what vendors state, W54.2 added the alias table and per-vendor tokenizers, W54.3 renamed `model_params` and gave the line/generation/version/gradation decomposition columns and filters. `provider` now names four companies across 60 of 66 rows where it was 12 rows of `openai` alone. |
| W54.1 | High | **Complete** | Decode the model and harness evidence vendors state and Codess discarded. Measured over all 22 Projects after re-ingesting every one: `surface_kind` is Claude `api` 327 / `desktop` 30 / `cli` 23 where it was the constant `cli`; `reasoning_effort` is on 29 configurations where Claude held zero; `provider` on 29 of 64 against 12 of 58; Cursor `interaction_mode` splits `chat` 17 / `agent` 17. `decode_audit.py` and `field_coverage.py --fail-on-gap` both exit 0 over all 22. **Three findings changed the work.** **Codex's provider gap was structural, not a missed read**: provider is stated on `session_meta` and model on `turn_context`, so a decoder reading one record at a time could never see both, and the store held a provider-only row beside a model-only one. The Session-level provider now seeds the accumulated parameters; a provider stated on a turn still wins. **Claude `entrypoint` counts invert between denominators**: `sdk-cli` is 1,656 records but 328 of 370 Sessions, `claude-desktop` 43,186 records but 24 Sessions, because SDK Sessions are many and short. `surface_kind` is a `sessions` column, so the per-Session count is the operative one and this item's original "about half ran under Desktop" is a record-count claim. `sdk-cli` therefore maps to `api`, not `cli`: those records carry `promptSource: sdk`, so mapping on the token would flatten the majority class. Promoting a per-record field to a Session column is licensed by a check -- across 370 Sessions none mixed two values. **Cursor states `unifiedMode` twice**, as a word on the composer and an integer on bubbles; across 3,984 bubbles the integer co-occurred with `agent` alone, so only the composer's spelling is read rather than storing a constant of unknown meaning. `composerData` was not opened by the ingest path at all. One baseline entry added: `reasoning_effort` is now empty for Cursor alone, which states no effort field -- the strength sits inside its model label, and recovering it is W54.2's parsing. | Each stated field is stored; `surface_kind` and `reasoning_effort` vary where the source varies; a re-ingest changes the counts. |
| W54.2 | Normal | **Complete** | Add per-vendor model tokenizers and a loadable alias table. **Delivered as `schema/model-aliases.json` and `codess/model_names.py`.** The table is data, so a new model name is a data edit; matching is case-folded and on the whole string, not a substring, because `claude` appears inside Cursor's model names as well as Anthropic's. A name absent from the table falls to a per-vendor tokenizer, and one that matches neither is recorded **unresolved rather than guessed** -- the vendor label is retained verbatim in every case and only the derived fields are left null, so "not recognized" stays distinct from "has none". **The token orders are genuinely incompatible**: Claude writes `claude-opus-4-8` (gradation second, hyphenated version, no strength), Cursor `claude-4.6-opus-high-thinking` (version second, gradation third, strength appended), Codex `gpt-5.6-sol` (dotted version, gradation last), so a Cursor-shaped label offered as Claude is refused rather than misread. **Two sentinels are declared rather than parsed**: Claude's `<synthetic>` (62 real records, meaning no model answered) and Cursor's `default` (the absence of an explicit choice, which is not an unknown model). **A decode gap surfaced while measuring**: Cursor states its model on `composerData.modelConfig.modelName` for every composer, while the bubble `modelInfo` the adapter read carries one on **3,044 of 188,904 records**, and the richer labels appear only on the composer -- `composer-2-fast` and `cursor-grok-4.5-high-fast` name a speed variant no bubble records. Reading the composer moved Cursor Sessions carrying a session-level model from 0 to 26 of 30 in one Project. | A Cursor label resolves to an API name and a strength; an unknown name is recorded unresolved rather than guessed; adding an alias is a data edit. |
| W54.3 | Normal | **Complete** | Correct the schema and names once W54.1 and W54.2 have shown what is real. **The model table is renamed and the decomposition has columns.** `model_configurations` is `model_params` -- independent parameters a user selects, not a configuration Codess composes -- with `model_config_id` following it and `source_config` becoming `source_params`. **A model name decomposes along three axes plus a gradation, and each now has a column and a filter**: `model_line` (`claude`, `gpt`, `grok`, `composer`), `model_generation` (the major step in whole numbers -- Claude has gone 3, 4, 5 and GPT likewise), `model_version` (the release within it, so `claude-opus-4-8` is version 4.8 of generation 4 exactly as `gpt-5.6` is 5.6 of generation 5), and `model_gradation` -- the capability level, which is what the old `model_family` actually held. **Gradations are ordered, lowest to highest**: Anthropic `haiku`, `sonnet`, `opus`, `fable`, `mythos`; OpenAI `luna`, `terra`, `sol`. The order is the vendor's own and is not recoverable from the names, so it is data and is exposed as a rank. `model_variant` holds superseded designators -- `codex`, `codex-max`, `latest` -- that occupied the gradation position historically, kept apart so one is not read as a current capability level. **Measured over all 22 Projects after regeneration**: of 66 rows, `provider` 60 across four companies, `model_line` 51, `model_generation` 51, `model_version` 51, `model_gradation` 23, `model_variant` 7. Every axis is queryable -- `--model-line claude` returns 4,747 Events in one Project, `--model-gradation opus` 1,898, `--model-version 4.8` 1,647. **`session_model_param_id`** replaced `default_model_config_id`, whose `default_` asserted a fallback role Claude never states. **Claude model fallback is decoded**: `model_consent_fallback` states `originalModel` and `fallbackModel`, and the record was previously skipped entirely. **Remaining, and moved to their owning items**: `vendor_name` recording the company (W51's batch) and Cursor's `default`, retained as a selection but with no column. | Vendor and provider are separately queryable; a fallback is visible as one; no model column is constant across every store. |

**Both words are already in the schema, and one is used wrongly.**
`sessions.vendor_name` holds `anthropic`, `openai`, `cursor` -- two companies and one product, since Cursor's vendor is Anysphere. `provider` on the model parameter rows is the right column for the second sense. So the fix is not new columns: it is `vendor_name` recording the company, and `provider` being decoded rather than left null. **Measured divergence.** Of Cursor's 10 model parameter sets, **5 name a provider other than its vendor** -- `claude-*` (Anthropic) and `grok-*` (xAI) against `composer-*` (Anysphere) -- so under one term the two facts are indistinguishable in half the rows of the one harness where it matters. Claude Code ran only `claude-*` and Codex only `gpt-*`, but Cursor ran three providers' models, so harness does not imply provider: an Anthropic Opus 4.6 turn was stored as `vendor_name=cursor` with `provider=NULL`. A model name carries a **line**, a **generation** and the **version** within it, a **gradation** (Opus, Sonnet, Haiku, Fable, Mythos for Anthropic; Sol, Terra, Luna for OpenAI), and sometimes a **strength**. The schema already declares the decomposition -- `provider`, `model_family`, `model_name_exact`, `model_revision`, `reasoning_effort`, `speed_tier` -- but **`model_family`, `model_revision`, and `speed_tier` are still zero**, because nothing parses a name: `opus`, `4.6`, and `high-thinking` sit opaque inside one string. **This supersedes earlier assumptions** formed before the distinction was stated: W40's reading of the four identity terms, W51's treatment of `vendor_name`, and the W48 note that vendor tags are merely redundant -- `vendor_name` is not redundant, it is *wrong* for any harness running another provider's model. **Still to confirm for W54.2**: the family and strength vocabularies, and whether generation is parsed from the name or carried separately.
...
The searchable fields are additive: `provider`, `model_gradation`,
`model_revision`, and `reasoning_effort` already exist for them.

Cursor's `modelName` is **not** an API name: `claude-4.6-opus-high-thinking`
puts the version before the gradation and appends a strength, where Claude
Code's own `claude-opus-4-8` puts the gradation first and carries no strength.
...
**What each Source states, and what remains to be read.** The stated fields
are decoded (W54.1); what is left is parsing and the two facts nothing
represents.

| Evidence | Claude Code | Codex | Cursor | State |
|---|---|---|---|---|
| Exact model name | `message.model` | `payload.model` | `modelInfo.modelName` (a label, not an API name) | decoded |
| Reasoning strength | `effort`, top-level | `effort` (`high`, `medium`) | `maxMode`; also embedded in the label | decoded except Cursor's label |
| Provider | not stated | `model_provider` | not stated | decoded |
| Harness surface | `entrypoint` | `originator` | `agentBackend` (`cursor-agent`) | decoded except Cursor |
| Interaction mode | -- | `collaboration_mode`, `approval_policy`, `sandbox_policy` | `unifiedMode`, `forceMode` | Cursor decoded; Codex's three remain |
| Model substitution | `originalModel` + `fallbackModel` | -- | `modelConfig.modelName` = `default` | **not represented** |

**Two facts nothing currently represents.** `originalModel` with
...
unknown model. Both are W54.3's, and both are worth a decision rather than a
silent flattening.

**On aliases.** A supplied mapping file is the right shape for Cursor:
`modelConfig` gives `{modelName, maxMode}` and `unifiedMode` the interaction
mode, but neither yields a provider, and `claude-4.6-opus-high-thinking`
needs an alias table to reach `claude-opus-4-6` with strength `high`. Case
folding before comparison, per-vendor tokenizers, and a loadable alias table
are accepted; the table should be data rather than code so a vendor's new
model name does not require a release. **`model` records what the vendor
gave, verbatim; `model_api` is added for Cursor to carry the derived API
name**, so the label and the identifier are never confused. Mythos exists but
is not generally available, so the family vocabulary is `opus`, `sonnet`,
`haiku`, `fable`, and `mythos`, the last named and not generally available.
...
| W53 | High | **Complete** | Compile the settled-names list before any rename lands. **Delivered as [CoNames](CoNames.md)**, a top-level reference: the four facts a name may refer to (vendor, harness, surface, provider), the vendor and harness designator table naming each concept's DB column, code location, and values, the `_id` suffix rule, the plurality rule, and every accepted rename in one table. **What compiling it settled or corrected.** Claude has **five spellings for three things** -- `cc` (adapter module, store filename, CLI token), `Claude` (adapter key), `claude-code` (product), `anthropic.claude-code` (source system), `anthropic` (vendor) -- and `claude` also appears inside Cursor's model names, so designators are matched exactly rather than by substring. **A new defect surfaced from W54.1's decode**: `sessions.harness_name` is the constant `claude-code-cli` for all 380 Claude Sessions, including the **357 whose decoded `surface_kind` is `api` or `desktop`** -- the constant embeds a surface claim the decoded column now contradicts, so it is a decode fix rather than a rename. **Two of W53's own measurements did not reproduce and are corrected there**: the peer-schema survey (56 tables across four local databases) cannot be re-run, since three of the four are gone from disk and the survivor has 6 tables, so the plurality rule now rests on CoSchema itself -- 19 of 24 tables plural, all five exceptions mass nouns; and Cursor's provider divergence is 3 of 6 distinct model names in the current stores rather than 5 of 10. **The earlier proposal to pluralize `event_content` is withdrawn**: it and `event_artifacts` disagree because the nouns differ, not the convention. | One document names every designator once; a reviewer can check any proposed rename against it; the six rename items reference it rather than each other. |
