...
|---|---|---|
| **A. Fixed now** | Landed in the working tree, with tests. Carries no work item. | Group 1, decode correctness, is complete: every field the three vendors state is decoded, the model name is decomposed into line, generation, version, and gradation, and coverage is reported from the store rather than from run-time counters. Names are settled in [CoNames](CoNames.md), and `harness_name` no longer carries a surface suffix -- `claude-code-cli` contradicted the decoded `surface_kind` on 351 of 374 Claude Sessions. What each change established is recorded where it is checkable -- `schema/field-coverage-baseline.json` for accepted gaps, `schema/model-aliases.json` for the model vocabulary -- rather than retold here. |
