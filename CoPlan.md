...
| **C. Fix later** | Accepted, unblocked, not started. Grouped and ordered below. | W54, W54.1, W54.2, W54.3, W04, W07, W08, W09, W13, W47, W48, W49, W50, W51, W53, W14, W15, W18, W19, W21, W24, W28, W31, W32, W37, W38, W39, W41, W42 |
...
|---|---|---|
| 0. Fix the model | **W54**, and within it **W54.1** (decode what is stated), **W54.2** (aliases and tokenizers), **W54.3** (schema and names) | Harness, vendor, provider, and model are four facts, not one. Every name below refers to one of them, so naming them before the distinction is settled fixes the wrong words. |
...
| W54.1 | High | Planned | Decode the model and harness evidence vendors already state and Codess discards. **Claude Code records `effort` on 14,050 records and stores `reasoning_effort` for zero rows** -- the same field is read from Codex and ignored from Claude. **Claude `entrypoint` distinguishes `cli` (24,089), `claude-desktop` (23,792), and `sdk-cli` (187), while `sessions.surface_kind` is the constant `cli` for all 400 Claude Sessions**, so about half are recorded as CLI Sessions that ran under Claude Desktop -- the defect W40 fixed for Codex, unnoticed in Claude because the constant looked plausible. Also decode Codex `model_provider`, Cursor `unifiedMode` and `modelConfig.maxMode`. No schema change and no parsing: every value is stated by the vendor. | Each stated field is stored; `surface_kind` and `reasoning_effort` vary where the source varies; a re-ingest changes the counts. |
