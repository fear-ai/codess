...
| W56 | Normal | Planned | Unify the read and write connection paths, and settle `foreign_keys`. **The read path has one owner and the write path has none.** `fileio.open_readonly` owns the read-only URI, `query_only`, and `busy_timeout` for every reader, and 3.5.4 records why it had to: only some call sites had set `query_only`, so the guarantee varied by site rather than by intent. Writing never got the same treatment. `store.connect` sets `row_factory` and `PRAGMA foreign_keys = ON`; the four raw `sqlite3.connect` calls -- `store.init_db`, `snapshot._backup_store`, `snapshot._logical_counts`, `raw_store._sqlite_backup` -- set neither. **What the gap is, stated precisely.** SQLite defaults `foreign_keys` to `0` per connection, so the pragma is a property of the connection and not of the file. Measured on a real store: **18 of 24 tables carry foreign keys, and a raw connection accepts an orphan Event (`session_id` naming no Session) that `store.connect` refuses with `FOREIGN KEY constraint failed`.** **No current path reaches that**, which is the reason this is not a defect today: all four raw sites write only to `store_meta`, which has no foreign keys, or use SQLite's `backup()` API, which copies pages and bypasses constraints by design. The exposure is that nothing prevents the next write added to one of these connections from being unchecked, and it would fail silently -- an orphan row is accepted, not reported. **What to decide.** Either one `open_writable` primitive owns the write-side contract as `open_readonly` owns the read-side one, or each of the four states why it differs. The second is defensible for the two `backup()` sites, where constraints are genuinely irrelevant; it is weaker for `init_db`, which creates the store every later writer depends on. Do not collapse them merely because they look alike -- 3.5.3's warning about false consolidation applies to connections as much as to constants. Also assess whether `row_factory` belongs with the opener rather than being set per caller, and whether `except sqlite3.Error` handlers should name a Codess error at the layer boundary so a caller is not catching a driver exception across it. | One primitive owns each of the read and write connection contracts, or each exception states why it differs at the call site; a store cannot be opened for writing without the constraint enforcement its writes assume; a test fails if a new raw `sqlite3.connect` appears outside the primitives. |
